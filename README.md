...
CD your terminal into the logicgate-code-challenge folder.

Run the sort.rb file.

```ruby sort.rb```

JSON will get printed out into your terminal

## Summary

I chose to do an initial sort on the data, first sorting all of the animals by their ```"barn"``` color, rather than going straight for sorting them into their final barns. I did this so that I could find the total number of barns per color that would be needed, that way I could evenly distribute the animals. If I got to attempt this challenge again with more time I would be interested to see if I could sort the animals into their placeholder barns on the first pass of iterating through the data. If I could reduce the number of loops it would run more efficiently and handle larger datasets better. I would also like to see if I could eliminate the need to hardcore the index number in certain instances.
